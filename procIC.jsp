...
<div class="container-fluid">
    <div class="row">
		...
		String qry1 = "insert into CHANGEIMPACT values('"+crno+"', TO_CHAR(sysdate, 'dd/MM/yyyy'), '"+
               impactarea+"', '"+
               bcp+"', '"+
               recommended+"', '0', '', '0', '', '"+status+"')"; 
int execStatus = st.executeUpdate(qry1);
if( 1== execStatus){
out.println("Change Impact Record for CR No.<font color=\"blue\">" + crno +"</font> has been successfully raised..");

if(status.equals("final")){
qry1 = "update CHANGEREQUEST set CIMPDUECHANGE='1' where CRNO='"+crno+"'";
execStatus = st.executeUpdate(qry1);
if( 1== execStatus){
   out.println("Change Impact Record for CR No.<font color=\"blue\">" + crno +"</font> has been updated."); 
}
}
}else
out.println("<font color=\"red\">Error: Change Impact Analysis Report could not be raised...</font>");
